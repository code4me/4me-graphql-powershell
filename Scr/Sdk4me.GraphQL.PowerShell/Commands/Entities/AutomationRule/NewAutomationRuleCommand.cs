...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.New, "AutomationRule")]
    [OutputType(typeof(AutomationRule))]
    public class NewAutomationRuleCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            AutomationRuleCreatePayload result = client.Sdk4meClient.Mutation(input, new AutomationRuleQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "NewAutomationRuleError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
