...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.New, "ServiceOffering")]
    [OutputType(typeof(ServiceOffering))]
    public class NewServiceOfferingCommand : PSCmdlet
    {
        /// <summary>
        ...
        /// </summary>
        [Parameter(Mandatory = true, Position = 70, ValueFromPipeline = false, ValueFromPipelineByPropertyName = true)]
        [ValidateNotNull]
        ...
        protected override void ProcessRecord()
        {
            ...
            ServiceOfferingCreatePayload result = client.Sdk4meClient.Mutation(input, new ServiceOfferingQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "NewServiceOfferingError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
