...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    public class SetProjectCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            ProjectUpdatePayload result = client.Sdk4meClient.Mutation(input, new ProjectQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "SetProjectError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
