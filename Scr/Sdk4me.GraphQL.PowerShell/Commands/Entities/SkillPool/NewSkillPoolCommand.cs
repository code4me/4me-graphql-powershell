...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.New, "SkillPool")]
    [OutputType(typeof(SkillPool))]
    public class NewSkillPoolCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            SkillPoolCreatePayload result = client.Sdk4meClient.Mutation(input, new SkillPoolQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "NewSkillPoolError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
