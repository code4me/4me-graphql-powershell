...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "SurveyResponse")]
    [OutputType(typeof(SurveyResponse))]
    public class SetSurveyResponseCommand : PSCmdlet
    {
        /// <summary>
        ...
        public string[] AnswersToDelete { get; set; } = Array.Empty<string>();
        ...
        protected override void ProcessRecord()
        {
            ...
            if (MyInvocation.BoundParameters.ContainsKey("AnswersToDelete"))
            {
                input.AnswersToDelete = AnswersToDelete.ToList();
            }
            ...
            SurveyResponseUpdatePayload result = client.Sdk4meClient.Mutation(input, new SurveyResponseQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "SetSurveyResponseError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
