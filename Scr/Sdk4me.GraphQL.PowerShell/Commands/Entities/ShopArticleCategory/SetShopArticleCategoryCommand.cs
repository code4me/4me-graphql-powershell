...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "ShopArticleCategory")]
    [OutputType(typeof(ShopArticleCategory))]
    public class SetShopArticleCategoryCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            ShopArticleCategoryUpdatePayload result = client.Sdk4meClient.Mutation(input, new ShopArticleCategoryQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "SetShopArticleCategoryError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
