...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.New, "Broadcast")]
    [OutputType(typeof(Broadcast))]
    public class NewBroadcastCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            BroadcastCreatePayload result = client.Sdk4meClient.Mutation(input, new BroadcastQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "NewBroadcastError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
