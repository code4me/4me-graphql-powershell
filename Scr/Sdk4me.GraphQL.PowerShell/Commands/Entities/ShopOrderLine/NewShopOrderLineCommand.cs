...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.New, "ShopOrderLine")]
    [OutputType(typeof(ShopOrderLine))]
    public class NewShopOrderLineCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            ShopOrderLineCreatePayload result = client.Sdk4meClient.Mutation(input, new ShopOrderLineQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "NewShopOrderLineError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
