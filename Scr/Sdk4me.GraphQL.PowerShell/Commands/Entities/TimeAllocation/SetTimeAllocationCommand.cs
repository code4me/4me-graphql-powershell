...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "TimeAllocation")]
    [OutputType(typeof(TimeAllocation))]
    public class SetTimeAllocationCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            TimeAllocationUpdatePayload result = client.Sdk4meClient.Mutation(input, new TimeAllocationQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "SetTimeAllocationError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
