...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "Invoice")]
    [OutputType(typeof(Invoice))]
    public class SetInvoiceCommand : PSCmdlet
    {
        /// <summary>
        ...
        /// </summary>
        [Parameter(Mandatory = true, Position = 31, ValueFromPipeline = false, ValueFromPipelineByPropertyName = true)]
        [ValidateNotNull]
        ...
        protected override void ProcessRecord()
        {
            ...
            InvoiceUpdatePayload result = client.Sdk4meClient.Mutation(input, new InvoiceQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "SetInvoiceError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
