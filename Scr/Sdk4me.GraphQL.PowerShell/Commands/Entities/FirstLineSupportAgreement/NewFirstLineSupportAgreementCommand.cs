...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.New, "FirstLineSupportAgreement")]
    [OutputType(typeof(FirstLineSupportAgreement))]
    public class NewFirstLineSupportAgreementCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            FirstLineSupportAgreementCreatePayload result = client.Sdk4meClient.Mutation(input, new FirstLineSupportAgreementQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "NewFirstLineSupportAgreementError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
