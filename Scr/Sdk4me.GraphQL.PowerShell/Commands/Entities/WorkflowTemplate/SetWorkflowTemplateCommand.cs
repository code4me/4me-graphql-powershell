...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "WorkflowTemplate")]
    [OutputType(typeof(WorkflowTemplate))]
    public class SetWorkflowTemplateCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            WorkflowTemplateUpdatePayload result = client.Sdk4meClient.Mutation(input, new WorkflowTemplateQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "SetWorkflowTemplateError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
