...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    public class NewReleaseCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            ReleaseCreatePayload result = client.Sdk4meClient.Mutation(input, new ReleaseQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "NewReleaseError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
