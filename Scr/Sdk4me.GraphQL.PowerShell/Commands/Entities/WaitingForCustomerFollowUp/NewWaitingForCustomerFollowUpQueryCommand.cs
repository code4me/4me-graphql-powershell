...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.New, "WaitingForCustomerFollowUpQuery")]
    [OutputType(typeof(WaitingForCustomerFollowUp))]
    public class NewWaitingForCustomerFollowUpQueryCommand : PSCmdlet
    {
        /// <summary>
        ...
        [ValidateNotNull]
        public WaitingForCustomerRuleQuery WaitingForCustomerRules { get; set; } = new();
        ...
        [ValidateNotNull]
        public QueryFilter<WaitingForCustomerFollowUpQuery>[] Filters { get; set; } = Array.Empty<QueryFilter<WaitingForCustomerFollowUpQuery>>();
        ...
        {
            WaitingForCustomerFollowUpQuery retval = ID == null || ID == string.Empty ? new() : new(ID);
            ...
            if (MyInvocation.BoundParameters.ContainsKey("WaitingForCustomerRules"))
            {
                retval.SelectWaitingForCustomerRules(WaitingForCustomerRules);
            }

            if (MyInvocation.BoundParameters.ContainsKey("Filters"))
            {
                foreach (QueryFilter<WaitingForCustomerFollowUpQuery> filter in Filters)
                {
