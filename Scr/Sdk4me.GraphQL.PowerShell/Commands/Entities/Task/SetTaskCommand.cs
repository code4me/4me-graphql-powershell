...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    public class SetTaskCommand : PSCmdlet
    {
        /// <summary>
        ...
        public string[] ApprovalsToDelete { get; set; } = Array.Empty<string>();
        ...
        /// </summary>
        [Parameter(Mandatory = true, Position = 44, ValueFromPipeline = false, ValueFromPipelineByPropertyName = true)]
        [ValidateNotNull]
        ...
        protected override void ProcessRecord()
        {
            ...
            if (MyInvocation.BoundParameters.ContainsKey("ApprovalsToDelete"))
            {
                input.ApprovalsToDelete = ApprovalsToDelete.ToList();
            }
            ...
            TaskUpdatePayload result = client.Sdk4meClient.Mutation(input, new TaskQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "SetTaskError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
