...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    public class NewSiteCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            SiteCreatePayload result = client.Sdk4meClient.Mutation(input, new SiteQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "NewSiteError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
