...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "ServiceInstance")]
    [OutputType(typeof(ServiceInstance))]
    public class SetServiceInstanceCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            ServiceInstanceUpdatePayload result = client.Sdk4meClient.Mutation(input, new ServiceInstanceQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "SetServiceInstanceError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
