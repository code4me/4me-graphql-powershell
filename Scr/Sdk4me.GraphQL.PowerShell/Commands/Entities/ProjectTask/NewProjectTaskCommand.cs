...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.New, "ProjectTask")]
    [OutputType(typeof(ProjectTask))]
    public class NewProjectTaskCommand : PSCmdlet
    {
        /// <summary>
        ...
        /// </summary>
        [Parameter(Mandatory = true, Position = 35, ValueFromPipeline = false, ValueFromPipelineByPropertyName = true)]
        [ValidateNotNull]
        ...
        protected override void ProcessRecord()
        {
            ...
            ProjectTaskCreatePayload result = client.Sdk4meClient.Mutation(input, new ProjectTaskQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "NewProjectTaskError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
