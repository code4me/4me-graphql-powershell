...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.New, "Workflow")]
    [OutputType(typeof(Workflow))]
    public class NewWorkflowCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            WorkflowCreatePayload result = client.Sdk4meClient.Mutation(input, new WorkflowQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "NewWorkflowError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
