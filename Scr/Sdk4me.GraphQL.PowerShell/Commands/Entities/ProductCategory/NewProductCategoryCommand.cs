...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.New, "ProductCategory")]
    [OutputType(typeof(ProductCategory))]
    public class NewProductCategoryCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            ProductCategoryCreatePayload result = client.Sdk4meClient.Mutation(input, new ProductCategoryQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "NewProductCategoryError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
