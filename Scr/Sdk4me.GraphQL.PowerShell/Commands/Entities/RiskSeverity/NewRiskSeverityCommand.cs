...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.New, "RiskSeverity")]
    [OutputType(typeof(RiskSeverity))]
    public class NewRiskSeverityCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            RiskSeverityCreatePayload result = client.Sdk4meClient.Mutation(input, new RiskSeverityQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "NewRiskSeverityError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
