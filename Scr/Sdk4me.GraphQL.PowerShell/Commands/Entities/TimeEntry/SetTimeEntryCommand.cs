...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "TimeEntry")]
    [OutputType(typeof(TimeEntry))]
    public class SetTimeEntryCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            TimeEntryUpdatePayload result = client.Sdk4meClient.Mutation(input, new TimeEntryQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "SetTimeEntryError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
