...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.New, "TeamQuery")]
    [OutputType(typeof(Team))]
    public class NewTeamQueryCommand : PSCmdlet
    {
        /// <summary>
        ...
        [ValidateNotNull]
        public PersonQuery ConfigurationManager { get; set; } = new();
        ...
        [ValidateNotNull]
        public PersonQuery Coordinator { get; set; } = new();
        ...
        [ValidateNotNull]
        public PersonQuery Members { get; set; } = new();
        ...
        [ValidateNotNull]
        public ScrumWorkspaceQuery ScrumWorkspace { get; set; } = new();
        ...
        [ValidateNotNull]
        public QueryFilter<TeamQuery>[] Filters { get; set; } = Array.Empty<QueryFilter<TeamQuery>>();
        ...
        {
            TeamQuery retval = ID == null || ID == string.Empty ? new() : new(ID);
            ...
            if (MyInvocation.BoundParameters.ContainsKey("ConfigurationManager"))
            {
                retval.SelectConfigurationManager(ConfigurationManager);
            }
            if (MyInvocation.BoundParameters.ContainsKey("Coordinator"))
            {
                retval.SelectCoordinator(Coordinator);
            }
            ...
            if (MyInvocation.BoundParameters.ContainsKey("Members"))
            {
                retval.SelectMembers(Members);
            }
            if (MyInvocation.BoundParameters.ContainsKey("RemarksAttachments"))
            {
                retval.SelectRemarksAttachments(RemarksAttachments);
            }
            if (MyInvocation.BoundParameters.ContainsKey("ScrumWorkspace"))
            {
                retval.SelectScrumWorkspace(ScrumWorkspace);
            }
            ...
            {
                foreach (QueryFilter<TeamQuery> filter in Filters)
                {
