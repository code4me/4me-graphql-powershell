...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.New, "Survey")]
    [OutputType(typeof(Survey))]
    public class NewSurveyCommand : PSCmdlet
    {
        /// <summary>
        /// Name of the survey.
        /// </summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            SurveyCreatePayload result = client.Sdk4meClient.Mutation(input, new SurveyQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "NewSurveyError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
