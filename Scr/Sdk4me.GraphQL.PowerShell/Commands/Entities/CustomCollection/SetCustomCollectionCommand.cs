...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "CustomCollection")]
    [OutputType(typeof(CustomCollection))]
    public class SetCustomCollectionCommand : PSCmdlet
    {
        /// <summary>
        ...
        public string[] CollectionElementsToDelete { get; set; } = Array.Empty<string>();
        ...
        protected override void ProcessRecord()
        {
            ...
            if (MyInvocation.BoundParameters.ContainsKey("CollectionElementsToDelete"))
            {
                input.CollectionElementsToDelete = CollectionElementsToDelete.ToList();
            }
            ...
            CustomCollectionUpdatePayload result = client.Sdk4meClient.Mutation(input, new CustomCollectionQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "SetCustomCollectionError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
