...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.New, "ShortUrl")]
    ...
    public class NewShortUrlCommand : PSCmdlet
    {
        ...
        protected override void ProcessRecord()
        {
            ...
            ShortUrlCreatePayload result = client.Sdk4meClient.Mutation(input, new ShortUrlQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "NewShortUrlError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
