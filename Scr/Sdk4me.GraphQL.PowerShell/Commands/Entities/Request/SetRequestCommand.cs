...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "Request")]
    [OutputType(typeof(Request))]
    public class SetRequestCommand : PSCmdlet
    {
        /// <summary>
        ...
        public string[] TagsToDelete { get; set; } = Array.Empty<string>();
        ...
        public string[] WatchesToDelete { get; set; } = Array.Empty<string>();
        ...
        /// </summary>
        [Parameter(Mandatory = true, Position = 51, ValueFromPipeline = false, ValueFromPipelineByPropertyName = true)]
        [ValidateNotNull]
        ...
        protected override void ProcessRecord()
        {
            ...
            if (MyInvocation.BoundParameters.ContainsKey("TagsToDelete"))
            {
                input.TagsToDelete = TagsToDelete.ToList();
            }
            ...
            if (MyInvocation.BoundParameters.ContainsKey("WatchesToDelete"))
            {
                input.WatchesToDelete = WatchesToDelete.ToList();
            }
            ...
            RequestUpdatePayload result = client.Sdk4meClient.Mutation(input, new RequestQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "SetRequestError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
