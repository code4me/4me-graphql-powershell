...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "ConfigurationItem")]
    [OutputType(typeof(ConfigurationItem))]
    public class SetConfigurationItemCommand : PSCmdlet
    {
        /// <summary>
        ...
        public string[] CiRelationsToDelete { get; set; } = Array.Empty<string>();
        ...
        /// </summary>
        [Parameter(Mandatory = true, Position = 48, ValueFromPipeline = false, ValueFromPipelineByPropertyName = true)]
        [ValidateNotNull]
        ...
        protected override void ProcessRecord()
        {
            ...
            if (MyInvocation.BoundParameters.ContainsKey("CiRelationsToDelete"))
            {
                input.CiRelationsToDelete = CiRelationsToDelete.ToList();
            }
            ...
            ConfigurationItemUpdatePayload result = client.Sdk4meClient.Mutation(input, new ConfigurationItemQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "SetConfigurationItemError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
