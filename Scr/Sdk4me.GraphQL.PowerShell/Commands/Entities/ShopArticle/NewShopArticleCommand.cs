...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.New, "ShopArticle")]
    [OutputType(typeof(ShopArticle))]
    public class NewShopArticleCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            ShopArticleCreatePayload result = client.Sdk4meClient.Mutation(input, new ShopArticleQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "NewShopArticleError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
