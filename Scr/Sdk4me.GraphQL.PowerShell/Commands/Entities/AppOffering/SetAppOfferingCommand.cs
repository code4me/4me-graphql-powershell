...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "AppOffering")]
    [OutputType(typeof(AppOffering))]
    public class SetAppOfferingCommand : PSCmdlet
    {
        /// <summary>
        ...
        public string[] ScopesToDelete { get; set; } = Array.Empty<string>();
        ...
        protected override void ProcessRecord()
        {
            ...
            if (MyInvocation.BoundParameters.ContainsKey("ScopesToDelete"))
            {
                input.ScopesToDelete = ScopesToDelete.ToList();
            }
            ...
            AppOfferingUpdatePayload result = client.Sdk4meClient.Mutation(input, new AppOfferingQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "SetAppOfferingError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
