...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "TimesheetSetting")]
    [OutputType(typeof(TimesheetSetting))]
    public class SetTimesheetSettingCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            TimesheetSettingUpdatePayload result = client.Sdk4meClient.Mutation(input, new TimesheetSettingQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "SetTimesheetSettingError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
