...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.New, "CalendarQuery")]
    [OutputType(typeof(Calendar))]
    public class NewCalendarQueryCommand : PSCmdlet
    {
        /// <summary>
        ...
        [ValidateNotNull]
        public CalendarHourQuery CalendarHours { get; set; } = new();
        ...
        [ValidateNotNull]
        public HolidayQuery Holidays { get; set; } = new();
        ...
        [ValidateNotNull]
        public QueryFilter<CalendarQuery>[] Filters { get; set; } = Array.Empty<QueryFilter<CalendarQuery>>();
        ...
        {
            CalendarQuery retval = ID == null || ID == string.Empty ? new() : new(ID);
            ...
            if (MyInvocation.BoundParameters.ContainsKey("CalendarHours"))
            {
                retval.SelectCalendarHours(CalendarHours);
            }
            if (MyInvocation.BoundParameters.ContainsKey("Holidays"))
            {
                retval.SelectHolidays(Holidays);
            }

            if (MyInvocation.BoundParameters.ContainsKey("Filters"))
            {
                foreach (QueryFilter<CalendarQuery> filter in Filters)
                {
