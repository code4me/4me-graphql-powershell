...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "KnowledgeArticle")]
    [OutputType(typeof(KnowledgeArticle))]
    public class SetKnowledgeArticleCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            KnowledgeArticleUpdatePayload result = client.Sdk4meClient.Mutation(input, new KnowledgeArticleQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "SetKnowledgeArticleError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
