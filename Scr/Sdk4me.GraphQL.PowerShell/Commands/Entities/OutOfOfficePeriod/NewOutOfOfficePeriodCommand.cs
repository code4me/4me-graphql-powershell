...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.New, "OutOfOfficePeriod")]
    [OutputType(typeof(OutOfOfficePeriod))]
    public class NewOutOfOfficePeriodCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            OutOfOfficePeriodCreatePayload result = client.Sdk4meClient.Mutation(input, new OutOfOfficePeriodQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "NewOutOfOfficePeriodError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
