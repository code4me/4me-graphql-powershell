...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "CustomCollectionElement")]
    [OutputType(typeof(CustomCollectionElement))]
    public class SetCustomCollectionElementCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            CustomCollectionElementUpdatePayload result = client.Sdk4meClient.Mutation(input, new CustomCollectionElementQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "SetCustomCollectionElementError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
