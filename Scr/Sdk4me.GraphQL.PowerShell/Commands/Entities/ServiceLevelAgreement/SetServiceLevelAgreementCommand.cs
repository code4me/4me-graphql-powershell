...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "ServiceLevelAgreement")]
    [OutputType(typeof(ServiceLevelAgreement))]
    public class SetServiceLevelAgreementCommand : PSCmdlet
    {
        /// <summary>
        ...
        public string[] EffortClassRateIDsToDelete { get; set; } = Array.Empty<string>();
        ...
        public string[] StandardServiceRequestActivityIDsToDelete { get; set; } = Array.Empty<string>();
        ...
        protected override void ProcessRecord()
        {
            ...
            if (MyInvocation.BoundParameters.ContainsKey("EffortClassRateIDsToDelete"))
            {
                input.EffortClassRateIDsToDelete = EffortClassRateIDsToDelete.ToList();
            }
            ...
            if (MyInvocation.BoundParameters.ContainsKey("StandardServiceRequestActivityIDsToDelete"))
            {
                input.StandardServiceRequestActivityIDsToDelete = StandardServiceRequestActivityIDsToDelete.ToList();
            }
            ...
            ServiceLevelAgreementUpdatePayload result = client.Sdk4meClient.Mutation(input, new ServiceLevelAgreementQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "SetServiceLevelAgreementError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
