...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.New, "ServiceCategory")]
    [OutputType(typeof(ServiceCategory))]
    public class NewServiceCategoryCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            ServiceCategoryCreatePayload result = client.Sdk4meClient.Mutation(input, new ServiceCategoryQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "NewServiceCategoryError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
