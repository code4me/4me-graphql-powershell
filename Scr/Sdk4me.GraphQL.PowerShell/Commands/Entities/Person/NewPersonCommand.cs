...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    public class NewPersonCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            PersonCreatePayload result = client.Sdk4meClient.Mutation(input, new PersonQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "NewPersonError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
