...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "RequestTemplate")]
    [OutputType(typeof(RequestTemplate))]
    public class SetRequestTemplateCommand : PSCmdlet
    {
        /// <summary>
        ...
        /// </summary>
        [Parameter(Mandatory = true, Position = 33, ValueFromPipeline = false, ValueFromPipelineByPropertyName = true)]
        [ValidateNotNull]
        ...
        protected override void ProcessRecord()
        {
            ...
            RequestTemplateUpdatePayload result = client.Sdk4meClient.Mutation(input, new RequestTemplateQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "SetRequestTemplateError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
