...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.New, "PdfDesign")]
    [OutputType(typeof(PdfDesign))]
    public class NewPdfDesignCommand : PSCmdlet
    {
        /// <summary>
        /// Name of the design.
        /// </summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            PdfDesignCreatePayload result = client.Sdk4meClient.Mutation(input, new PdfDesignQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "NewPdfDesignError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
