...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.New, "ProductBacklog")]
    [OutputType(typeof(ProductBacklog))]
    public class NewProductBacklogCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            ProductBacklogCreatePayload result = client.Sdk4meClient.Mutation(input, new ProductBacklogQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "NewProductBacklogError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
