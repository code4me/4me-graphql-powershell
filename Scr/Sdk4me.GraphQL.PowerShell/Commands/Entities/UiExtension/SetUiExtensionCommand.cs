...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "UiExtension")]
    [OutputType(typeof(UiExtension))]
    public class SetUiExtensionCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            UiExtensionUpdatePayload result = client.Sdk4meClient.Mutation(input, new UiExtensionQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "SetUiExtensionError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
