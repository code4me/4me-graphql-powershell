...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "ProjectTaskTemplate")]
    [OutputType(typeof(ProjectTaskTemplate))]
    public class SetProjectTaskTemplateCommand : PSCmdlet
    {
        /// <summary>
        ...
        /// </summary>
        [Parameter(Mandatory = true, Position = 34, ValueFromPipeline = false, ValueFromPipelineByPropertyName = true)]
        [ValidateNotNull]
        ...
        protected override void ProcessRecord()
        {
            ...
            ProjectTaskTemplateUpdatePayload result = client.Sdk4meClient.Mutation(input, new ProjectTaskTemplateQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "SetProjectTaskTemplateError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
