...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.New, "EffortClassQuery")]
    [OutputType(typeof(EffortClass))]
    public class NewEffortClassQueryCommand : PSCmdlet
    {
        /// <summary>
        ...
        [ValidateNotNull]
        public TimesheetSettingQuery TimesheetSettings { get; set; } = new();
        ...
        [ValidateNotNull]
        public QueryFilter<EffortClassQuery>[] Filters { get; set; } = Array.Empty<QueryFilter<EffortClassQuery>>();
        ...
        {
            EffortClassQuery retval = ID == null || ID == string.Empty ? new() : new(ID);
            ...
            if (MyInvocation.BoundParameters.ContainsKey("TimesheetSettings"))
            {
                retval.SelectTimesheetSettings(TimesheetSettings);
            }

            if (MyInvocation.BoundParameters.ContainsKey("Filters"))
            {
                foreach (QueryFilter<EffortClassQuery> filter in Filters)
                {
