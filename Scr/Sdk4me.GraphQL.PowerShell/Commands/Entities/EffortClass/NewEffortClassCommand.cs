...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.New, "EffortClass")]
    [OutputType(typeof(EffortClass))]
    public class NewEffortClassCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            EffortClassCreatePayload result = client.Sdk4meClient.Mutation(input, new EffortClassQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "NewEffortClassError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
