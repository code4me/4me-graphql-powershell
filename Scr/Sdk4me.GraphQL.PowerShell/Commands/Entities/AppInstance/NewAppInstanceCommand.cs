...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.New, "AppInstance")]
    [OutputType(typeof(AppInstance))]
    public class NewAppInstanceCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            AppInstanceCreatePayload result = client.Sdk4meClient.Mutation(input, new AppInstanceQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "NewAppInstanceError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
