...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "Organization")]
    [OutputType(typeof(Organization))]
    public class SetOrganizationCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            OrganizationUpdatePayload result = client.Sdk4meClient.Mutation(input, new OrganizationQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "SetOrganizationError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
