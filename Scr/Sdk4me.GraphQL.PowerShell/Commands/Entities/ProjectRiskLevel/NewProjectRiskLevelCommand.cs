...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.New, "ProjectRiskLevel")]
    [OutputType(typeof(ProjectRiskLevel))]
    public class NewProjectRiskLevelCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            ProjectRiskLevelCreatePayload result = client.Sdk4meClient.Mutation(input, new ProjectRiskLevelQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "NewProjectRiskLevelError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
