...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "WorkflowType")]
    [OutputType(typeof(WorkflowType))]
    public class SetWorkflowTypeCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            WorkflowTypeUpdatePayload result = client.Sdk4meClient.Mutation(input, new WorkflowTypeQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "SetWorkflowTypeError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
