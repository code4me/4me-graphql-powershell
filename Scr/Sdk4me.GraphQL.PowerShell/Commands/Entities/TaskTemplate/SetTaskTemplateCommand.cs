...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "TaskTemplate")]
    [OutputType(typeof(TaskTemplate))]
    public class SetTaskTemplateCommand : PSCmdlet
    {
        /// <summary>
        ...
        {
            TaskTemplateUpdateInput  input = new();
            ...
            TaskTemplateUpdatePayload result = client.Sdk4meClient.Mutation(input, new TaskTemplateQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "SetTaskTemplateError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
