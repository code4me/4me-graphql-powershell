...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.New, "ReservationOffering")]
    [OutputType(typeof(ReservationOffering))]
    public class NewReservationOfferingCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            ReservationOfferingCreatePayload result = client.Sdk4meClient.Mutation(input, new ReservationOfferingQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "NewReservationOfferingError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
