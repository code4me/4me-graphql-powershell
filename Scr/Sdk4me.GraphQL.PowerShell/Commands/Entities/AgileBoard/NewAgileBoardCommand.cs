...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.New, "AgileBoard")]
    [OutputType(typeof(AgileBoard))]
    public class NewAgileBoardCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            AgileBoardCreatePayload result = client.Sdk4meClient.Mutation(input, new AgileBoardQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "NewAgileBoardError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
