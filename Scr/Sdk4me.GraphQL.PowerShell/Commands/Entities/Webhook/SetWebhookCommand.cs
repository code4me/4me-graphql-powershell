...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.Set, "Webhook")]
    [OutputType(typeof(Webhook))]
    public class SetWebhookCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            WebhookUpdatePayload result = client.Sdk4meClient.Mutation(input, new WebhookQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "SetWebhookError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
