...
namespace Sdk4me.GraphQL.PowerShell.Commands
{
    ...
    /// </summary>
    [Cmdlet(VerbsCommon.New, "ProjectCategory")]
    [OutputType(typeof(ProjectCategory))]
    public class NewProjectCategoryCommand : PSCmdlet
    {
        /// <summary>
        ...
        protected override void ProcessRecord()
        {
            ...
            ProjectCategoryCreatePayload result = client.Sdk4meClient.Mutation(input, new ProjectCategoryQuery().Select(Properties), false).ConfigureAwait(true).GetAwaiter().GetResult();
            PowerShellTraceListener.UnregisterCmdlet();
            if (result.IsError())
            {
                foreach (ValidationError error in result.Errors ?? new())
                {
                    WriteError(new ErrorRecord(new Exception(error.Message), "NewProjectCategoryError", ErrorCategory.InvalidOperation, input));
                }
                return;
            }
